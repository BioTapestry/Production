...
public class IntercellFree extends NodeRenderBase {
  ...
  */
  ...
  /***************************************************************************
   **
   ** Render the intercellular node at the designated location
   */
  ...
  public void render(ModelObjectCache cache, GenomeItem item, Intersection selected, DataAccessContext rcx, Object miscInfo) {
    ...
    Color vac = getVariableActivityColor(item, col1Val, false, rcx.getDisplayOptsSource().getDisplayOptions());
    ...
    Color vac2 = getVariableActivityColor(item, col2Val, false, rcx.getDisplayOptsSource().getDisplayOptions());
    ...
    path = new GeneralPath(); // path.reset();
    ...
    pathShape = new ModelObjectCache.SegmentedPathShape(DrawMode.DRAW, col2, pathStroke, path);
    group.addShape(pathShape, majorLayer, minorLayer);
    ...
    Color finalTextCol = (textGhosted) ? Color.LIGHT_GRAY : textCol;
  ...
  public void setGroupBounds(BoundShapeContainer group, Genome genome, GenomeItem item, DataAccessContext rcx) {
  ...
  public Intersection intersects(GenomeItem item, Point2D pt, DataAccessContext rcx, Object miscInfo) {
    ...
    String name = item.getName();
    if ((name != null) && (!name.trim().equals(""))) {
      if (isTextCandidate(rcx.getFrc(), FontManager.MEDIUM, np.getFontOverride(), name, TEXT_PAD_, dx, 
    ...
    if (numPads > defaultPads) {
      ...
      double quickRad = 1.25 * (EXTRA_PAD_START_OFFSET_ + ((double)((extraPads + 1) / 2) * PAD_SPACING_));
    ...
    bounds = UiUtil.padTheRect(bounds, ((double)PAD_WIDTH_) / 2.0);
  ...
  */
  ...
  private Vector2D getLaunchPadOffsetGuts(int orient) {
    AffineTransform rot = getRotationTransform(orient);
    Vector2D rightDepart = new Vector2D(ARROW_OFFSET_, 0.0);
    return (rightDepart.doTransform(rot));
  }
  ...
  public Vector2D getLandingPadOffset(int padNum, GenomeItem item, int sign, 
    ...
    if (padNum >= 0) {
      ...
      double xOff = -(EXTRA_PAD_START_OFFSET_ + ((double)(((-padNum) - 1) / 2) * PAD_SPACING_));
  ...
  */
  ...
  public List<Integer> getNearbyPads(GenomeItem item, int startPad, Layout layout) {
    ...
    if (!haveExtraPads(item) || (startPad == 0)) {
      return (null);
    }
  ...
  */
  ...
  private List<Intersection.PadVal> calcPadIntersects(GenomeItem item, Point2D pt, DataAccessContext rcx) {
  ...
  */
  ...
  private Rectangle2D extraPadRectangleForPads(int numPads, Point2D origin, int nodeType) {
    ...
    double extraSize = EXTRA_PAD_START_OFFSET_ + ((double)(extraPerSide - 1) * PAD_SPACING_);
  ...
  private ModelObjectCache.PushRotationWithOrigin getRotationTransform(int orient, Point2D origin) {
    float x = (float)origin.getX();
    float y = (float)origin.getY();
  ...
  private AffineTransform getRotationTransformOld(int orient, Point2D origin) {
    float x = (float)origin.getX();
    float y = (float)origin.getY();
    AffineTransform trans = new AffineTransform();
    double radians = 0.0;
  ...
  */
  ...
  private void paintExtra(CommonCacheGroup group, Color fillColor, Rectangle2D extraRect, double diameter, boolean isSelected) {
  	Integer majorLayer = NodeRenderBase.NODE_MAJOR_LAYER;
  	Integer minorLayer = NodeRenderBase.NODE_MINOR_LAYER;
    ...
    path.lineTo((float)extraRect.getMaxX() - (float)STROKE_, (float)extraRect.getCenterY() - radius);
