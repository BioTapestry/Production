...
public class DialogSupport {
  ...
  */ 
   ...
   */ 
   ...
   public int installTrueLabelJCompPair(JPanel cp, JLabel typeLab, JComponent tf1, 
                                        JLabel lab2, JComponent tf2, int rowNum, int colWidth, boolean gottaGrow) { 
     int halfCol = colWidth / 2;
  ...
  */ 
  ...
  public int addLabeledWidgetWithInsets(JPanel cp, JLabel label, JComponent comp, boolean fixHeight, boolean flushLeft,
                                        int inst, int insl, int insb, int insr, 
                                        int rowNum, int columns) {
    double vFac = (fixHeight) ? 0.0 : 1.0;
    int eorw = (flushLeft) ? UiUtil.W : UiUtil.E;
    UiUtil.gbcSet(gbc_, 0, rowNum, 1, 1, UiUtil.NONE, 0, 0, inst, insl, insb, 5, eorw, 0.0, vFac);
    cp.add(label, gbc_);
    UiUtil.gbcSet(gbc_, 1, rowNum++, columns - 1, 1, UiUtil.HOR, 0, 0, inst, 5, insb, insr, UiUtil.CEN, 1.0, vFac);    
    cp.add(comp, gbc_);        
    return (rowNum);
  }
   ...
   */ 
   ...
   public int addLabeledButtonPair(JPanel cp, JLabel label, JRadioButton but1, JRadioButton but2, int rowNum, int columns) {
     
     UiUtil.gbcSet(gbc_, 0, rowNum, columns - 2, 1, UiUtil.NONE, 0, 0, 5, 5, 5, 5, UiUtil.E, 0.0, 1.0);       
     cp.add(label, gbc_);
     
     UiUtil.gbcSet(gbc_, columns - 2, rowNum, 1, 1, UiUtil.HOR, 0, 0, 5, 5, 5, 5, UiUtil.CEN, 1.0, 1.0);
     cp.add(but1, gbc_);
     
     UiUtil.gbcSet(gbc_, columns - 1, rowNum++, 1, 1, UiUtil.HOR, 0, 0, 5, 5, 5, 5, UiUtil.CEN, 1.0, 1.0);
     cp.add(but2, gbc_);              
     return (rowNum);
   }
  ...
  /***************************************************************************
  **
  ** Add a large, labeled scrolled component:
  */ 
   
   public int addLabeledScrolledWidget(JPanel cp, String labelKey, JComponent comp, int rowNum, int columns, int scrollRows) {
  
     JLabel label = new JLabel(rMan_.getString(labelKey), JLabel.LEFT);
     UiUtil.gbcSet(gbc_, 0, rowNum++, columns, 1, UiUtil.HOR, 0, 0, 5, 5, 5, 5, UiUtil.E, 1.0, 0.0);
     cp.add(label, gbc_);    
 
     JScrollPane jsp = new JScrollPane(comp); 

     UiUtil.gbcSet(gbc_, 0, rowNum, columns, scrollRows, UiUtil.BO, 0, 0, 5, 5, 5, 5, UiUtil.CEN, 1.0, 1.0);
     cp.add(jsp, gbc_);
     return (rowNum + scrollRows);
   }
