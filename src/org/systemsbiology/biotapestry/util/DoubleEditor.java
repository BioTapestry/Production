...
public class DoubleEditor extends JTextField implements TableCellEditor, NumberEditor {
  ...
  public DoubleEditor(HandlerAndManagerSource hams, JFrame topWindow) {
    this(hams, topWindow, false);
  }
  
  public DoubleEditor(HandlerAndManagerSource hams, JFrame topWindow, boolean blanksOK) {
    hams_ = hams;
  ...
  public static void triggerWarning(HandlerAndManagerSource hams, JFrame top) {
  ...
  public static void triggerWarningWithTag(HandlerAndManagerSource hams, JFrame top, String tag, String val) {
    ResourceManager rMan = hams.getRMan();
  ...
  private void editingStopped() {
    if (!neverEdited_) {  // Added because we can stop editing on a field never edited
      ...
      if (!(blanksOK_ && (text != null) && (text.trim().equals("")))) {
        ...
        if (!test.valid) {
          setText((origValue_ == null) ? null : origValue_.textValue);
          triggerWarning(hams_, topWindow_);
        }
      }
    }
    ChangeEvent cev = new ChangeEvent(this);
