...
public class AffineCombination {
  ...
  */
  ...
  public static List<Double> getWeights(List<Point2D> points, Point2D point) {
    ...
    Map<Point2D, List<Integer>> indexMap = createIndexMap(points);
  ...
  */
  ...
  private static Map<Point2D, List<Integer>> createIndexMap(List<Point2D> points) {
    HashMap<Point2D, List<Integer>> retval = new HashMap<Point2D, List<Integer>>();
    int size = points.size();
    for (int i = 0; i < size; i++) {
      Point2D point = points.get(i);
      List<Integer> indices = retval.get(point);
      if (indices == null) {
  ...
  */
  ...
  private static List<Double> distributeWeights(Map<Point2D, List<Integer>> indexMap, List<Point2D> ordered, 
                                                ArrayList[] weights, double sum, int origSize) {
  
    double[] preRetval = new double[origSize];
    int size = ordered.size();
    for (int i = 0; i < size; i++) {
      Point2D point = ordered.get(i);
      ...
      List<Integer> indices = indexMap.get(point);
  ...
  */
  ...
  private static List<Point2D> orderPoints(List<Point2D> points, Point2D ref) {
    ...
    for (int i = 0; i < size; i++) {
      OrderedPoint oPoint = working.get(i);
  ...
  */
  ...
  private static class OrderedPoint implements Comparable<OrderedPoint> {
    ...
    public boolean equals(Object other) {
      ...
      return (this.compareTo((OrderedPoint)other) == 0);
    } 
