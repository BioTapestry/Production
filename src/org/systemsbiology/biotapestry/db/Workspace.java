...
public class Workspace implements Cloneable {
  ...
  */
  ...
  public static Workspace setToFreeModelBounds(Rectangle modelSize) {
    Rectangle padded = UiUtil.padTheRect(modelSize, PADDING);
    Point2D centerPt = getAllModelCenter(padded);
    int x = (int)Math.round(centerPt.getX() - (padded.getWidth() / 2.0));
    int y = (int)Math.round(centerPt.getY() - (padded.getHeight() / 2.0));    
    return (new Workspace(new Rectangle(x, y, padded.width, padded.height)));
  }
